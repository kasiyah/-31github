# Common Keys
# Define a function which takes two dictionaries as parameters and 
# merge them and sum the values of common keys
...
b. Add the value from dict2 to the current value (or 0, if the key is not in result) and update the result 
dictionary with the new value for the key. The get() and update operations take O(1) average time complexity.

Return the merged dictionary result.

Time complexity:
The overall time complexity of this function is O(n + m), where n is the number of elements in dict1 and m is the number 
of elements in dict2. The copy() method takes O(n) time, and the loop iterates m times with O(1) operations inside the loop.

Space complexity:
The space complexity of this function is O(n + m) in the worst case, where all keys in dict1 and dict2 are distinct, 
and the merged dictionary has n + m elements. In the best case, where dict1 and dict2 have the same keys, 
the space complexity is O(n) (or O(m), whichever is larger), as the merged dictionary has the same number of elements 
