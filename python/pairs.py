...
result.append(f"{arr[i]}+{arr[j]}") 
- If the sum of the elements at index i and index j equals the target sum, append a formatted string 
containing the pair to the result list.

return result 
- After iterating through all pairs in the array, return the result list 
...
This is because the function uses nested loops, and for each element in the array, it checks every other 
element in the array for possible pairs.

The space complexity of the pair_sum function is O(n), where n is the length of the input array arr. 
The additional space used in this function is the result list, which stores the pairs that add up to 
the target sum. In the worst case, the number of pairs that sum up to the target value is proportional 
